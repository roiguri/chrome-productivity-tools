...
function createOverlay(input, matches, text) {
    ...
    // Position it exactly over the input
    overlayContainer.style.position = 'absolute';
    overlayContainer.style.top = `${window.scrollY + rect.top}px`;
    overlayContainer.style.left = `${window.scrollX + rect.left}px`;
    ...
    // We copy ALL relevant font/spacing styles to ensure text aligns perfectly.
    ...
    if (input.tagName === 'INPUT') {
        ...
        mirror.style.overflow = 'hidden'; // internal mirror scroll is handled via transform usually, or we just let it flow
    }

    // IMPORTANT: Mirror content needs to be constructed carefully.
    // We iterate through matches and construct a DOM with <span>s for errors.

    // Sort matches by offset just in case
    ...
    matches.forEach(match => {
        ...
        // Color coding: Rule category
        // LanguageTool categories: TYPOS, GRAMMAR, STYLE, etc.
        ...
        // Add interaction for tooltip (pointer-events: auto on the span itself)
...
function handleCorrectionClick(e, match, input) {
    // Optional: Make clicking the underline show the tooltip immediately/persistently
    // Already handled by hover mostly, but good for mobile or explicit action.
}
...
function applyCorrection(span, replacement) {
    ...
    // Use setRangeText to preserve other content and handle selection
    // Note: setRangeText is supported on HTMLInputElement and HTMLTextAreaElement
    if (typeof input.setRangeText === 'function') {
        input.setRangeText(replacement, offset, offset + length, 'select');

        // Dispatch input event so other scripts (and our own clearOverlay) know it changed
        ...
        // Fallback for contentEditable or others (not fully supported in this MVP scope)
